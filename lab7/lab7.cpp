...
public:
  ...
  bool addBook(Book arr[],T t,long key);
  void print(Book arr[]);
};
...
template <class T>
Book<T>::Book():title(""),ISBN(0) {}

template <class T> 
Book<T>:: Book(T t, long num){
    title = t;
    ISBN = num;
}

template <class T>
int Book<T>::hashFun(long key) {
    return key%ARRSIZE;
}

template <class T>
bool Book<T>::addBook(Book arr[],T t, long key) {
